...
<p>{{ __('By creating an account or using Distributed Bank services, you agree to these Terms of Service. If you do not agree, please do not use our platform.') }}</p>

<h2>2. {{ __('Account Registration') }}</h2>
<p>{{ __('To use Distributed Bank services, you must:') }}</p>
<ul>
...
<p>{{ __('Distributed Bank provides digital banking services including:') }}</p>
<ul>
...
<p>{{ __('All transfers on Distributed Bank use an escrow model. Funds are debited from the sender\'s account and held securely until the recipient accepts the transfer. Recipients may decline transfers, in which case funds are returned to the sender. Pending transfers may be cancelled by the sender before acceptance.') }}</p>

<h2>5. {{ __('Account Security') }}</h2>
<p>{{ __('You are responsible for maintaining the confidentiality of your account credentials. Distributed Bank implements automatic account lockout after 5 consecutive failed login attempts (30-minute lock period). You must notify us immediately of any unauthorized access.') }}</p>

<h2>6. {{ __('Prohibited Activities') }}</h2>
<p>{{ __('You may not use Distributed Bank services for:') }}</p>
<ul>
...
<p>{{ __('Distributed Bank reserves the right to suspend, freeze, or terminate accounts that violate these terms, are involved in suspicious activity, or fail to complete required KYC verification within the specified timeframe.') }}</p>

<h2>8. {{ __('Limitation of Liability') }}</h2>
<p>{{ __('Distributed Bank is a demonstration platform. While we implement real security measures, banking services are simulated. Distributed Bank shall not be liable for any indirect, incidental, or consequential damages arising from the use of our services.') }}</p>
