...
<div style="max-width:600px;">
    ...
    <div class="card p-6 animate-fade-in-up" style="margin-top:16px;">
        <div style="text-align:center;margin-bottom:32px;">
            ...
            <div style="font-size:40px;font-weight:800;color:var(--text-primary);">${{ number_format($amount, 2) }}</div>
            <div class="text-muted text-sm mt-2">{{ __('Transfer Amount') }}</div>
        </div>
        ...
        <div class="card p-4 mb-4">
            ...
            <div class="text-sm text-secondary">{{ __('Balance:') }} ${{ number_format($fromAccount->balance, 2) }}</div>
        </div>
        ...
        <div class="card p-4 mb-4" style="border-color:rgba(0,212,255,0.15);">
            ...
            <div class="receipt-row" style="padding:8px 0;">
                ...
                <span class="receipt-value">${{ number_format($amount, 2) }}</span>
            </div>
            <div class="receipt-row" style="padding:8px 0;">
                <span class="receipt-label">{{ __('Fee') }}</span>
                <span class="receipt-value text-green">$0.00</span>
            </div>
            <div class="receipt-row" style="padding:8px 0;border-bottom:none;">
                ...
                <span class="receipt-value font-bold text-cyan">${{ number_format($fromAccount->balance - $amount, 2) }}</span>
            </div>
        </div>
