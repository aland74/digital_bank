...
<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;background:var(--bg-primary);">
    <div style="width:100%;max-width:460px;">
        ...
        <div class="card" style="padding:32px;">
            <h2 style="font-size:20px;font-weight:700;color:var(--text-primary);margin:0 0 4px;">{{ __('Create your account') }}</h2>
            <p style="font-size:14px;color:var(--text-muted);margin:0 0 24px;">{{ __('Join NexusBank for secure digital banking') }}</p>
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div class="form-group">
                    ...
                    <select name="branch" class="form-select" required>
                        <option value="" disabled {{ old('branch') ? '' : 'selected' }}>{{ __('Select your branch...') }}</option>
                        <option value="erbil" {{ old('branch') === 'erbil' ? 'selected' : '' }}>🏙️ {{ __('Erbil') }}</option>
                        <option value="sulaimaniyah" {{ old('branch') === 'sulaimaniyah' ? 'selected' : '' }}>🏔️ {{ __('Sulaimaniyah') }}</option>
                        <option value="duhok" {{ old('branch') === 'duhok' ? 'selected' : '' }}>🌄 {{ __('Duhok') }}</option>
                    </select>
                </div>
            ...
            @endphp
            @if($hasGoogleConfig)
                <a href="{{ route('auth.google.redirect') }}" style="display:flex;align-items:center;justify-content:center;gap:10px;width:100%;padding:10px 16px;border:1px solid var(--border);border-radius:var(--radius);font-size:14px;font-weight:500;color:var(--text-primary);text-decoration:none;">
...
<div id="google-sandbox-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;align-items:center;justify-content:center;padding:16px;">
    <div style="background:var(--bg-white);border-radius:16px;padding:28px;width:100%;max-width:380px;">
